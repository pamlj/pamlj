...
const events = {
    ...
    onChange_model_terms: function(ui) {
        console.log("modelterms changed")
        filterModelTerms(ui, this);
        updateRandomSupplier(ui,this);
        updateEffectSizes(ui,this);
    ...
    onChange_re_changed: function(ui) {

     console.log("re_changed");
     updateEffectSizes(ui, this);
      
    },
    onEvent_addRandomTerm: function(ui) {
     console.log("addRandomTerm do nothing");
...
var updateEffectSizes = function(ui, context) {
    ...
    var  currentList  = context.cloneArray(ui.re_sizes.value(), []);
    var  reList    = context.cloneArray(ui.re.value(), []);
         reList    = reList.map(alist => alist.map(aname => {
                       var q= aname.slice(0,-1).join("*")+" | "+aname.slice(aname.length -1)
                       return(q)
        })).flat();
    ...
    for (let i = 0; i < reList.length; i++) {
        let found = null;
        for (let j = 0; j < currentList.length; j++) {
            if (currentList[j].name === reList[i]) {
                found = currentList[j];
                break;
            }
        }
        if (found === null)
            list3.push({ name: reList[i], value: "" });
        else
            list3.push(found)
    }
    ui.re_sizes.setValue(list3);
};
