mixpowerClass <- R6::R6Class(
  "mixpowerClass",
  inherit = mixpowerBase,
  private=list(
    .dispatcher=NULL,
    .data_machine=NULL,
    .runner=NULL,
    ...
    .init=function() {
      ...
      class(private$.results) <- c('pamlj', class(private$.results))
      ...
      runner<-MPRunner$new(self$options,dispatcher,data_machine)

      ### info table ###
      aSmartObj<-SmartTable$new(self$results$info,runner)
      ladd(private$.smartObjs)<-aSmartObj


      ## R2 table ###
      aSmartObj<-SmartTable$new(self$results$main$r2,runner)
      aSmartObj$spaceBy<-"model"
      ladd(private$.smartObjs)<-aSmartObj
      

      ### anova table ###
      aSmartObj<-SmartTable$new(self$results$main$anova,runner)
      ladd(private$.smartObjs)<-aSmartObj
      
      ### estimates table ###
      aSmartObj<-SmartTable$new(self$results$main$coefficients,runner)
      ladd(private$.smartObjs)<-aSmartObj
      

      ### random variances table
      aSmartObj<-SmartTable$new(self$results$main$random,runner)
      ladd(private$.smartObjs)<-aSmartObj
      
      ### random covariances table
      aSmartObj<-SmartTable$new(self$results$main$randomcov,runner)
      aSmartObj$activateOnData<-TRUE
      ladd(private$.smartObjs)<-aSmartObj
      
      ### random variances lrt table
      aSmartObj<-SmartTable$new(self$results$main$ranova,runner)
      ladd(private$.smartObjs)<-aSmartObj
      ...
      ######## plotting class #######
#      plotter_machine<-Plotter$new(self$options,runner,self$results)
...
#      private$.plotter_machine<-plotter_machine
    ...
    .run=function() {
      ...
      private$.checkpoint()
       ...
       private$.runner$savePredRes(self$results) 
      # 
...
#      }  
      ...
      if (self$options$.interface=="R")
              self$results$.setModel(private$.runner$model)
...
.sourcifyOption = function(option) {
  ...
  if (option$name=="nested_re") {
      ...
      if (!is.something(self$options$nested_re))
          return('')
      form<-stats::as.formula(fromb64(private$.runner$nested_formula64))  
