    ...
    switch(obj$options$model_type,
        ...
        threemeds = {
            ...
            if (obj$filled) {
                ...
                needed <- c(
                    "X to M1 (a1)", "M1 to Y (b1)", "X to M2 (a2)", "M2 to Y (b2)", "X to M3 (a3)",
...
.extrainfo.mediation <- function(obj) {
    ...
    infoparms <- list(
        n = "total sample size N=" %+% obj$data$n,
    ...
    if (is.something(obj$info$ryxpower)) {
        text <- text %+%
