...
Plotter <- R6::R6Class(
    ...
    public = list(
        ...
        plot_contour = function(image, ggthem, them) {
            ...
            res <- try_hard(
                filled.contour(data$x, data$y, data$z,
                    color.palette = image$state$plot_palette,
                    key.title = {
                        mtext("Power", 3, .5)
                    },
                    ylab = "",
        ...
        plot_custom = function(image, ggtheme, theme) {
            ...
            if (state$ticks) {
                ...
                p <- p + ggplot2::geom_label(
                    data = tdata, ggplot2::aes(x = x, y = y, label = round(y, digits = dig)),
    ...
    private = list(
            ...
            if (obj$options$mode == "medsimple") {
                ...
                goodvalues <- sapply(labs, function(x) {
                    if (is.na(x)) {
                        ...
                        return(paste0("=", format(x, digits = 3)))
                    }
                })
        ...
        .prepare_lav_diagram = function() {
            ...
            data <- private$.operator$data
            if (is.null(private$.operator$data$modelPop)) {
                return()
            }
