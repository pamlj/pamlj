...
DGINTRO<-"<b> Getting started</b>:<br> To produce a dataset useful for simulations, please start with an empty dataset.
        ...
        All variables are uncorrelated, their relationships can be defined in the power analysis."

DGBRIEF<-"<b>Available actions</b>:<br> One can use the produced data to estimate power parameters
...
####### mixed power analysis

MPINTRO<-"<b> Getting started</b>:<br> The analysis requires a dataset with the same structure
          of the expected data and the smallest numerosity: smallest number of clusters, smallest
          number of cases per cluster. 
          Different (larger) size for clusters and cases can be defined here.
          <br>
          All expected parameters can be defined in the input interface."

MPBRIEF<-"<b>Available actions</b>:<br> One can use the produced data to estimate power parameters
         of the mixed model. Although one can work directly on the active dataset, it is reccomanded
         to save the data as a <b>csv file</b>, so the data will not be altered in future sessions."
