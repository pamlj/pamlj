...
.powerbyes.default <- function(obj) {
    ...
    esList <- list(
        list(es = paste("0 <", obj$info$letter, greek_vector["leq"], probs_es[1])),
...
.extrainfo.default <- function(obj) {
    return()
}
...
.effectsize_run.default <- function(obj) {
    return()
}
...
.find_min_es.default <- function(obj, data) {
    if ("es" %in% obj$info$nochecks) {
        return(obj$info$esmin)
    }
